...
## Reading layer `PROVCenso2010' from data source `/home/mmvol/unidad-1-asignacion-1-datos-espaciales-en-r-MirelSIG/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
plot(prov[prov$PROV %in% '13', 'PROV'])
```

<img src="img/prov2-1.png" width="100%" />

``` r
Miprov <- prov[prov$PROV %in% '13', ]
Miprov
## Simple feature collection with 1 feature and 4 fields
## geometry type:  MULTIPOLYGON
## dimension:      XY
## bbox:           xmin: 293711.7 ymin: 2066774 xmax: 366129.2 ymax: 2143361
## epsg (SRID):    32619
## proj4string:    +proj=utm +zone=19 +datum=WGS84 +units=m +no_defs
##    PROV REG TOPONIMIA ENLACE                           geom
## 13   13  02   LA VEGA   0213 MULTIPOLYGON (((346936.3 21...
plot(Miprov)
```
...
## proj4string:    +proj=utm +zone=19 +datum=WGS84 +units=m +no_defs
##   PROV REG TOPONIMIA ENLACE                           geom
## 1   13  02   LA VEGA   0213 MULTIPOLYGON (((346936.3 21...
##           areaenm2
## 1 2292669004 [m^2]
`prov` %>% dplyr::filter(PROV %in% '13') %>% dplyr::select('areaenm2') %>% plot
```
...
``` r
`prov` %>% dplyr::filter(PROV %in% c('11', '13', '21')) %>% dplyr::select('areaenm2') %>% plot
```
...
## Reading layer `MunCenso2010' from data source `/home/mmvol/unidad-1-asignacion-1-datos-espaciales-en-r-MirelSIG/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
mimun <- mun %>% dplyr::filter(PROV %in% '13')
mimun %>% plot
```
